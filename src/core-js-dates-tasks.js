...
function getTime(date) {
  ...
  return `${formattedDate(currentDate.getHours())}:${formattedDate(currentDate.getMinutes())}:${formattedDate(currentDate.getSeconds())}`;
}
...
function getNextFriday(date) {
  const daysToFriday = [5, 4, 3, 2, 1, 7, 6];
  const currentDate = new Date(date);
  const millisecondsInDay = 24 * 60 * 60 * 1000;
  const nextFridayMilliseconds =
    currentDate.getTime() +
    daysToFriday[currentDate.getDay()] * millisecondsInDay;
  return new Date(nextFridayMilliseconds);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const currentDay = new Date(rightDateFormat(period.start));
  const end = new Date(rightDateFormat(period.end));
  const workDays = [];

  while (currentDay <= end) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (currentDay <= end) {
        workDays.push(formatData(currentDay));
